...
.EraseSprite
   ; now we've got the screen start address, and address of alien, lets erase it
   lda YScreenAddress ; put address in code below
   sta ErasePixelAddress+1      
   lda YScreenAddress+1        
   sta ErasePixelAddress+2     
   clc
   ;ok the main erase bit
   
.EraseXLoop
    ldy Height 
    dey  
    .EraseLoop   
       lda #0      ;value of byte to erase with
       .ErasePixelAddress
       sta $FFFF,X     ; dummy address, will be filled in by code
       ; are we at a boundary
       dex                  
       bpl EraseBoundary   ;   no, so carry on
         ;yes we moved to another character row, so we really want this to be the start of next screen row
         sec                     ; we do this by adding &279 to value to get to next row
         lda ErasePixelAddress+1
         sbc #$80
         sta ErasePixelAddress+1  
         lda ErasePixelAddress+2
         sbc #2
         sta ErasePixelAddress+2   
         ldx #7 ; reset X to 7 (bottom of this character row)
       .EraseBoundary
       dey
       bpl EraseLoop    ; have we finished a full column, go to plot loop if not
       dec Width
       beq EndEraseSprite   ; no more to plot, exit routine
     ; move to next column
     clc
     lda YScreenAddress
     clc
     ...
     sta ErasePixelAddress+2 
     ldx XStartOffset  
   jmp EraseXLoop ;never zero so always branches
   .EndEraseSprite
